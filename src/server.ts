...
const start = async () => {
  ...
  server.get("/api/account", async (_request, reply) => {
    ...
    if (query.count) {
      ...
      let account = await Account.queryAccountByAddress(
      ...
      if (query.page) {
        ...
        accounts = await Account.queryAccounts(
          (page - 1) * itemsPerPage,
          itemsPerPage,
          type
        );
      ...
      totalAccounts = await Account.queryAccountCountBetweenCycles(
        startCycle,
        endCycle
      );
      if (query.page) {
  ...
  server.get("/api/address", async (_request, reply) => {
    ...
    let account;
    if (query.accountType)
      account = await Account.queryAccountByAddress(
        query.address.toLowerCase(),
        parseInt(query.accountType)
      );
    else
      account = await Account.queryAccountByAddress(
        query.address.toLowerCase()
    ...
    if (accounts.length === 0) {
      try {
        ...
        const result = await axios.get(
          `http://${activeNode.ip}:${activeNode.port}/account/${query.address}`
        );
  ...
  server.get("/api/token", async (_request, reply) => {
    ...
    let tokens;
    if (query.address) {
      tokens = await Account.queryTokensByAddress(
        query.address.toLowerCase(),
        true
      );
      ...
      totalTokenHolders = await Account.queryTokenHolderCount(
    ...
    if (query.count) {
      ...
      if (query.txType) {
        if (
          ...
          txType = TransactionSearchType.StakeReceipt;
          const totalStakeTxs = await Transaction.queryTransactionCount(
            null,
            txType
          );
          res.totalStakeTxs = totalStakeTxs;
          txType = TransactionSearchType.UnstakeReceipt;
          const totalUnStakeTxs = await Transaction.queryTransactionCount(
            null,
            txType
          );
          res.totalUnStakeTxs = totalUnStakeTxs;
        } else {
          const totalRewardTxs = await Transaction.queryTransactionCount(
            null,
            txType
          );
    ...
    if (res.success) {
      ...
      if (transaction) {
        ...
        try {
          ...
          const activeNode = queryArchiver.data.nodeList[0];
          result = await axios.get(
            `http://${activeNode.ip}:${activeNode.port}/tx/${query.txHash}`
          );
  ...
  server.get("/api/archive/:counter", async (_request, reply) => {
    ...
    const archivedCycle = await ArchivedCycle.queryArchivedCycleByCounter(
      counter
    );
    const res = {
      success: true,
  ...
  server.get("/api/archive", async (_request, reply) => {
    ...
    if (query.start && query.end) {
      ...
      archivedCycles = await ArchivedCycle.queryAllArchivedCyclesBetween(
        from,
        to
      );
    }
    ...
    if (query.marker) {
      let archivedCycle = await ArchivedCycle.queryArchivedCycleByMarker(
        query.marker
      );
      if (archivedCycle) {
  ...
  server.get("/api/receipt", async (_request, reply) => {
    ...
    if (query.count) {
      ...
      console.log("endCycle", endCycle, "startCycle", startCycle);
      totalReceipts = await Receipt.queryReceiptCountBetweenCycles(
        startCycle,
        endCycle
      );
      const res: any = {
      ...
      receipts = await Receipt.queryReceipts(
        (page - 1) * itemsPerPage,
        itemsPerPage
      );
    } else {
      ...
      if (query.page) {
        ...
        if (query.type === "txs") {
          for (let i = 0; i < logs.length; i++) {
            const transaction = await Transaction.queryTransactionByHash(
              logs[i].txHash
            );
  ...
  server.get("/totalData", async (_request, reply) => {
    ...
    const totalAccounts = await Account.queryAccountCount(
  ...
  server.listen(Number(CONFIG.port.server), "0.0.0.0", async (err) => {
    ...
    console.log(
      "Shardeum explorer server is listening on port:",
      CONFIG.port.server
    );
  });
};
