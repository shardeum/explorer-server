  ...
  onMessage : async function(conn:SocketStream, message: any){
      ...
      if(method === 'subscribe'){
          try{
            ...
            console.log("adding subscription", (conn.socket as WebSocketWithId).id);
            addLogSubscriptions(address, topics, subscription_id, (conn.socket as WebSocketWithId).id);
            conn.socket.send(JSON.stringify({method: 'subscribe', success: true, subscription_id}))
            return
          }catch(e: any){
            conn.socket.send(JSON.stringify({method: 'subscribe', success: false, subscription_id, error: e.message}));
            return
          }
        }
      if(method === 'unsubscribe'){
        try{
          removeLogSubscription(subscription_id, (conn.socket as WebSocketWithId).id)
