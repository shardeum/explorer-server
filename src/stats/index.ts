...
export const initializeStatsDB = async () => {
    await db.init()
    await db.runCreate(
        'CREATE TABLE if not exists `validators` (`cycle` NUMBER NOT NULL UNIQUE PRIMARY KEY, `active` NUMBER NOT NULL, `timestamp` BIGINT NOT NULL)'
    )
    ...
    await db.runCreate(
        'CREATE TABLE if not exists `transactions` (`cycle` NUMBER NOT NULL UNIQUE PRIMARY KEY, `totalTxs` NUMBER NOT NULL, `timestamp` BIGINT NOT NULL)'
    )
