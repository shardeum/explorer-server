...
const start = async () => {
  ...
  server.get("/api/txs", async (_request, reply) => {
    ...
    if (lastestTenTxsKeys.length > 0) {
      lastestTenTxsKeys.forEach((key) =>
        lastestTenTxs.push(txStatusCollector.get(key))
      );
    }
    let res = {
      success: true,
  ...
  server.post("/tx/status", async (_request, reply) => {
    ...
    if (transactionInfos.length > 0) {
      ...
      if (txStatusCollector.size >= txCollectorMaxSize + batchCleanSize) {
        ...
        console.log(
          "txStatusCollector After cleanup",
          txStatusCollector.size,
          txKeysMap.size
        );
      }
    }
  ...
  server.listen(
    Number(CONFIG.port.rpc_data_collector),
    "0.0.0.0",
    async (err) => {
      if (err) {
      ...
      console.log(
        "Shardeum RPC data collector is listening on port:",
        CONFIG.port.rpc_data_collector
      );
    }
  );
};
